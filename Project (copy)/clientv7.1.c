  ...
  
{
    ...
    strftime(buffer, 26, "%Y/%m/%d %H:%M:%S", tm_infos);
    ...
    strcat(filname, "_");
    strcat(filname, SERVER_HOST_ADDR); 
    strcat(filname, "_");
    strcat(filname, argv[2]);
    strcat(filname, ".log");
    ...
    char in_buf[MAXSIZE];
    char ip[100];
    if(argc != 3) {printf("error: usage : sftp hostname \n"); exit(0);}
    no_writen = 0;
    num_blks = 0;
    num_last_blk = 0;
    len = strlen(argv[2])+1;
    ...
    //Try for second time with a new port. Still failed? then exit
    printf("Trying to use a new port\n");
    my_addr.sin_port = htons(CLIENT_PORT_ID+1);
    if (bind(sockid ,(struct sockaddr *) &my_addr,sizeof(my_addr)) < 0)
           {printf("client: bind  error :%d\n", errno); exit(0);
           }
    }
    ...
    //printf(inet_addr(SERVER_HOST_ADDR));
...
/*IF THERE IS A LAST PARTIALLY FILLED BLOCK, READ IT */
  ...
  /*FILE TRANSFER ENDS. CLIENT TERMINATES AFTER  CLOSING ALL ITS FILES
    ...
    if(milsecE>=milsecS)
    { 
   	 delay=(milsecE-milsecS)/1000;
    }
    if(milsecE<milsecS)
    { 
   	 delay=(milsecS-milsecE)/1000;
    }
    snprintf(delayS, 30, "%f", delay);
    strcat(mesg, "\n Delay is: ");
    strcat(mesg, delayS);
    strcat(mesg, "\n"); 
